 ...
 *
 */
public class TreasureLootPool {

	private final List<TreasureLootEntry> lootEntries;
	private final List<TreasureLootCondition> poolConditions;
	private RandomValueRange rolls;
	private RandomValueRange bonusRolls;
	private final String name;

	public TreasureLootPool(TreasureLootEntry[] lootEntriesIn, TreasureLootCondition[] poolConditionsIn, RandomValueRange rollsIn, RandomValueRange bonusRollsIn, String name) {
	 ...
	 */
	protected void createLootRoll(Collection<ItemStack> stacks, Random rand, TreasureLootContext context) {
		List<TreasureLootEntry> list = Lists.<TreasureLootEntry>newArrayList();
		int i = 0;

		for (TreasureLootEntry lootentry : this.lootEntries) {
//			Treasure.logger.debug("creating roll for loot entry -> {}", lootentry.entryName );
			if (TreasureLootConditionManager.testAllConditions(lootentry.conditions, rand, context)) {
		...
		if (i != 0 && !list.isEmpty()) {
			...
			for (TreasureLootEntry lootentry1 : list) {
	 ...
	 */
	public void generateLoot(Collection<ItemStack> stacks, Random rand, TreasureLootContext context) {
		if (TreasureLootConditionManager.testAllConditions(this.poolConditions, rand, context)) {
	...
	public TreasureLootEntry getEntry(String name) {
		for (TreasureLootEntry entry : this.lootEntries)
			if (name.equals(entry.getEntryName()))
				return entry;
		return null;
	}

	public TreasureLootEntry removeEntry(String name) {
		checkFrozen();
		for (TreasureLootEntry entry : this.lootEntries) {
			if (name.equals(entry.getEntryName())) {
	...
	public void addEntry(TreasureLootEntry entry) {
		checkFrozen();
		for (TreasureLootEntry e : this.lootEntries)
	...
	public static class Serializer implements JsonDeserializer<TreasureLootPool>, JsonSerializer<TreasureLootPool> {
		public TreasureLootPool deserialize(JsonElement element, Type p_deserialize_2_, JsonDeserializationContext p_deserialize_3_) throws JsonParseException {
			JsonObject jsonobject = JsonUtils.getJsonObject(element, "loot pool");
			String name = TreasureLootTableManager.readPoolName(jsonobject);
			TreasureLootEntry[] alootentry = (TreasureLootEntry[]) JsonUtils.deserializeClass(jsonobject, "entries", p_deserialize_3_, TreasureLootEntry[].class);
			TreasureLootCondition[] alootcondition = (TreasureLootCondition[]) JsonUtils.deserializeClass(jsonobject, "conditions", new TreasureLootCondition[0], p_deserialize_3_,
					TreasureLootCondition[].class);
			...
			return new TreasureLootPool(alootentry, alootcondition, randomvaluerange, randomvaluerange1, name);
		}

		public JsonElement serialize(TreasureLootPool p_serialize_1_, Type p_serialize_2_, JsonSerializationContext p_serialize_3_) {
