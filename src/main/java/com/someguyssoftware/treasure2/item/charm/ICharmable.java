...
/**
 * NOTE: if I go this route, then this means that an Item has the CHARM INVENTORY CAPABILITY but the CharmEntities may be empty.
 * ie. an item has the ABILITY to be charmed but isn't necessarily charmed.
 * All checks against hasCapability() then would also have to check against the capabilities CharmEntities state.
 * ie. !getCapability().getCharmEntities().isEmpty()
