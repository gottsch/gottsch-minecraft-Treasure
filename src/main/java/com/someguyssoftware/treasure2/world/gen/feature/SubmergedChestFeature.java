...
public class SubmergedChestFeature extends Feature<NoFeatureConfig> implements ITreasureFeature {
	...
	public boolean place(ISeedReader seedReader, ChunkGenerator generator, Random random, BlockPos pos, NoFeatureConfig config) {
		...
		for (Rarity rarity : TreasureData.RARITIES_MAP.get(LevelGenerators.SUBMERGED_CHEST)) {
...
//		Treasure.LOGGER.debug("chunks since dimension {} last chest -> {}, min chunks -> {}", dimensionName, chunksSinceLastDimensionChest.get(dimensionName), TreasureConfig.CHESTS.surfaceChests.minChunksPerChest.get());
		...
		if (chunksSinceLastDimensionChest.get(dimensionName.toString())> TreasureConfig.CHESTS.submergedChests.minChunksPerChest.get()) {
			...
			Rarity rarity = (Rarity) TreasureData.RARITIES_MAP.get(LevelGenerators.SUBMERGED_CHEST).get(random.nextInt(TreasureData.RARITIES_MAP.get(LevelGenerators.SUBMERGED_CHEST).size()));
...
//				if (chestConfig.getGenProbability() < prob) {
				...
				Treasure.LOGGER.debug("rarity -> {}", rarity);
				Treasure.LOGGER.debug("randcollection -> {}", TreasureData.CHEST_GENS.get(rarity, LevelGenerators.SUBMERGED_CHEST).getClass().getSimpleName());
				Treasure.LOGGER.debug("gen -> {}", TreasureData.CHEST_GENS.get(rarity, LevelGenerators.SUBMERGED_CHEST).next().getClass().getSimpleName());
				...
				result = generateChest(seedReader, generator, random, spawnCoords, rarity, TreasureData.CHEST_GENS.get(rarity, LevelGenerators.SUBMERGED_CHEST).next(), TreasureConfig.CHESTS.surfaceChests.configMap.get(rarity));
	 ...
	 * @return
	 */
	private GeneratorResult<ChestGeneratorData> generateChest(IServerLevel world, ChunkGenerator chunkGenerator, Random random, ICoords coords, Rarity rarity,
	 ...
	 * @return
	 */
	public GeneratorResult<ChestGeneratorData> generateSubmergedRuins(IServerLevel world, ChunkGenerator chunkGenerator, Random random, ICoords spawnCoords,
			IChestConfig config) {
