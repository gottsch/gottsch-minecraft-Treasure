...
public class SubmergedRuinGenerator implements IRuinGenerator<GeneratorResult<ChestGeneratorData>> {
	...
	@Override
	public GeneratorResult<ChestGeneratorData> generate(IServerWorld world, Random random, ICoords originalSpawnCoords,
	...
	public GeneratorResult<ChestGeneratorData> generate(IServerWorld world, Random random, ICoords originalSpawnCoords,
		...
		// TODO create a method selectTemplate() in abstract that will be overridden by
		// concrete classes, provided the archetype and type
		...
		if (holder == null) {
			holder = selectTemplate(world, random, originalSpawnCoords, StructureArchetype.SUBMERGED,
					StructureType.RUIN);
		}
		if (holder == null)
			return result.fail();
		...
		// NOTE these checks don't really belong in a generator as their task is to just
		// generate.
		// however, the template is unknown outside this call and thus the rotate,
		// placement, size and actual coords would be unknown.
		/**
		 * Environment Checks
		 */
		...
		for (int i = 0; i < 3; i++) {
			if (!WorldInfo.isSolidBase(world, actualSpawnCoords, templateSize.getX(), templateSize.getZ(),
					REQUIRED_BASE_SIZE)) {
				if (i == 2) {
					Treasure.LOGGER.debug(
							"Coords -> [{}] does not meet {}% solid base requirements for size -> {} x {}",
							REQUIRED_BASE_SIZE, originalSpawnCoords.toShortString(), templateSize.getX(),
							templateSize.getY());
					return result.fail();
				} else {
		...
		// TODO get the height of template, - don't get the immediate plane above spawn
		// because we want the template "buried" a little bit - unless the template
		// isn't very tall
		...
		Treasure.LOGGER.debug("checking for {} % water using offset of -> {} at coords -> {} for dimensions -> {} x {}",
				REQUIRED_WATER_SIZE, offset, actualSpawnCoords.add(0, offset, 0), templateSize.getX(),
				templateSize.getZ());
		if (!WorldInfo.isLiquidBase(world, actualSpawnCoords.add(0, offset, 0), templateSize.getX(),
				templateSize.getZ(), REQUIRED_WATER_SIZE)) {
			Treasure.LOGGER.debug("Coords -> [{}] does not meet {} % water base requirements for size -> {} x {}",
					actualSpawnCoords.toShortString(), REQUIRED_WATER_SIZE, templateSize.getX(), templateSize.getZ());
			return result.fail();
		}
		...
		// this is the coords that need to be supplied to the template generator to
		// allow
		// the structure to generator in the correct place
		originalSpawnCoords = new Coords(originalSpawnCoords.getX(), actualSpawnCoords.getY(),
				originalSpawnCoords.getZ());
		...
		if (decayRuleSet == null && holder.getDecayRuleSetLocation() != null
				&& holder.getDecayRuleSetLocation().size() > 0) {
			// create a decay processor with a random rule set
			decayRuleSet = TreasureDecayRegistry.getDecayManager().getRuleSetMap().get(holder.getDecayRuleSetLocation()
					.get(random.nextInt(holder.getDecayRuleSetLocation().size())).toString());
		...
		GeneratorResult<TemplateGeneratorData> genResult = generator.generate(world, random, decayProcessor, holder,
				placement, originalSpawnCoords);
		if (!genResult.isSuccess())
			return result.fail();

		Treasure.LOGGER.debug("submerged gen result -> {}", genResult);

		// interrogate info for spawners and any other special block processing (except
		// chests that are handler by caller
		List<BlockContext> bossChestContexts = (List<BlockContext>) genResult.getData().getMap()
				.get(TreasureTemplateRegistry.getMarkerBlock(StructureMarkers.BOSS_CHEST));
		List<BlockContext> chestContexts = (List<BlockContext>) genResult.getData().getMap()
				.get(TreasureTemplateRegistry.getMarkerBlock(StructureMarkers.CHEST));
		List<BlockContext> spawnerContexts = (List<BlockContext>) genResult.getData().getMap()
				.get(TreasureTemplateRegistry.getMarkerBlock(StructureMarkers.SPAWNER));
		List<BlockContext> proximityContexts = (List<BlockContext>) genResult.getData().getMap()
				.get(TreasureTemplateRegistry.getMarkerBlock(StructureMarkers.PROXIMITY_SPAWNER));
