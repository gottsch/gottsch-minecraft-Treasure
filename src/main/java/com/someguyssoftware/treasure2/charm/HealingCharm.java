...
public class HealingCharm extends Charm implements IHealing {
	private static String HEALING = "healing";
	...
	public boolean update(World world, Random random, ICoords coords, PlayerEntity player, Event event, final ICharmData data) {
		...
		if (world.getGameTime() % 20 == 0) {
			if (event instanceof LivingUpdateEvent) {
				if (data.getValue() > 0 && player.getHealth() < player.getMaxHealth() && player.isAlive()) {
	...
	public static class Builder extends Charm.Builder {
		...
		public Builder(Integer level) {
			super(ModUtils.asLocation(HEALING + level), HEALING, level);
		}
