...
public abstract class AbstractChestBlockEntityRenderer implements BlockEntityRenderer<AbstractTreasureChestBlockEntity>, ITreasureChestBlockEntityRenderer {
	 ...
	 *
	 * @param blockEntity the associated block entity
	 ...
	 * @param bufferSource    the buffer that you should render your model to
	...
	public void render(AbstractTreasureChestBlockEntity blockEntity, float partialTicks, PoseStack poseStack,
		...
		// update the lid rotation
		 ...
		 // update other rotations
		updateAdditionalRotation(poseStack, getModel(), blockEntity, partialTicks);

		VertexConsumer renderBuffer = material.buffer(bufferSource, getModel().getRenderType()); //RenderType::entitySolid);

		model.renderToBuffer(poseStack, renderBuffer, combinedLight, combinedOverlay, 1.0F, 1.0F, 1.0F, 1.0F, blockEntity);
		poseStack.popPose();

		renderAdditional(blockEntity, poseStack, bufferSource, combinedLight, combinedOverlay);
	}
	...
	/**
	 * a method to be overridden if needing additional rendering without changing the base chest rendering
	 */
	private void renderAdditional(AbstractTreasureChestBlockEntity blockEntity, PoseStack poseStack, MultiBufferSource bufferSource, int combinedLight, int combinedOverlay) {
		//	////////////// render the locks //////////////////////////////////////////////
