...
public class RarityBlockAssociationDataHandler extends SimpleJsonResourceReloadListener {

    private static final Gson GSON = new GsonBuilder().setPrettyPrinting().disableHtmlEscaping().create();

    private static final String DATA_DIRECTORY = "rarity_associations/block";

    public RarityBlockAssociationDataHandler() {
        super(GSON, DATA_DIRECTORY);
    }

    /**
     * this is the core method that gets called when a data pack reload happens.
     * parse the JSON files here.
     *
    ...
    protected void apply(Map<ResourceLocation, JsonElement> jsonElementMap, ResourceManager resourceManager, ProfilerFiller profilerFiller) {
        Treasure.LOGGER.info("loading rarity block associations from data packs...");

        RarityTagAssociationRegistry.Blocks.clear();
        ...
        jsonElementMap.forEach((location, jsonElement) -> {
            try {
                ...
                if (path.getNameCount() > 1) {
                    ...
                    RarityTagAssociationRegistry.Blocks.register(typeKey, association);
                }
...
//        Treasure.LOGGER.info("loaded {} rarity block associations.", TreasureWishables.getRarityAssociations().size());
    }
    ...
    public static void onAddReloadListener(AddReloadListenerEvent event) {
        ...
        event.addListener(new RarityBlockAssociationDataHandler());
    }
