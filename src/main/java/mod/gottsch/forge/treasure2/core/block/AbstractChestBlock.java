...
import mod.gottsch.forge.treasure2.core.chest.ILockSlot;
...
public abstract class AbstractChestBlock extends ModContainerBlock implements ITreasureBlock {
	...
	public ActionResultType use(BlockState state, World world, BlockPos pos, PlayerEntity player,
		...
		if (tileEntity != null) {
			...
			if (tileEntity.getMimic() != null) {
				spawnMimic(world, world.getRandom(), state, pos, player);
			}
	 ...
	 * @param player
	 */
	protected void spawnMimic(World level, Random random, BlockState state, BlockPos pos, PlayerEntity player) {
		AbstractTreasureChestTileEntity blockEntity = (AbstractTreasureChestTileEntity) level.getBlockEntity(pos);
		EntityType<?> entityType = ForgeRegistries.ENTITIES.getValue(blockEntity.getMimic());
		...
		MobEntity mob = spawn((ServerWorld)level, level.getRandom(), entityType, pos, player, yRot);
		if (mob != null) {
			...
			TreasureNetworking.simpleChannel.send(PacketDistributor.TRACKING_ENTITY.with(() -> mob), message);
		}
	}
	 ...
	 * @return
	 */
	protected MobEntity spawn(ServerWorld level, Random random, EntityType<?> entityType, BlockPos pos, LivingEntity target, float yRot) {
		double spawnX = pos.getX() + 0.5;
		double spawnY = pos.getY();
		double spawnZ = pos.getZ() + 0.5;
		if (!WorldInfo.isClientSide(level)) {
                if (EntitySpawnPlacementRegistry.checkSpawnRules(entityType, level, SpawnReason.SPAWNER, new BlockPos(spawnX, spawnY, spawnZ), level.getRandom())) {
 				MobEntity mob = (MobEntity)entityType.create(level);
