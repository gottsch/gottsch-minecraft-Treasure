 ...
 * 
 * @author Mark Gottschling on 8/26/2025
 *
 */
public class AdjustingWeightedCollection<T> {
	/*
	 ...
	 *
	 */
	public AdjustingWeightedCollection() {
		this(new Random());
	}
	 ...
	 * @param random
	 */
	public AdjustingWeightedCollection(Random random) {
	 ...
	 * @return
	 */
	public AdjustingWeightedCollection<T> add(Integer weight, T item) {
	 ...
	 * @return
	 */
	public AdjustingWeightedCollection<T> add(Pair<Integer, Integer> weightPair, T item) {
		original.put(item, weightPair);
		collection.add(weightPair.getRight(), item);
		return this;
	}

	public AdjustingWeightedCollection<T> only(List<T> items) {

		AdjustingWeightedCollection<T> newCollection = new AdjustingWeightedCollection<T>();
		this.collection.getMap().forEach((weight, item) -> {
			if (items.contains(item)) {
				newCollection.add(weight.intValue(), item);
			}
		});
	 ...
	 * @return
	 */
	public AdjustingWeightedCollection<T> adjustExcept(Integer incrementAmount, T type) {
		AdjustingWeightedCollection<T> col = new AdjustingWeightedCollection<>();
