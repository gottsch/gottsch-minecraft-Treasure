 ...
 *
 */
public class WitherFeatureGenerator2 implements IWitherFeatureGenerator {
	...
	@Override
	public int[][] generateGrid(IFeatureGenContext context, ICoords spawnCoords, IRarity rarity) {
		int[][] grid = new int[X_CHUNKS * 16][Z_CHUNKS * 16];
...
//		ICoords gridCenterCoords = new Coords(grid.length / 2 -1, 0, grid[0].length / 2 -1);
		...
		// calculate the number of supporting wither trees to generate
		int witherTrees = 20; // TODO

		// place wither trees centered around main tree
		// TODO should indicate large/medium/small already, or let the generation phase do that?
		// TODO let the generator handle that, this is just the map
		for (int num = 0; num < witherTrees; num++) {
			ICoords coords = generateRandomCoordsByRadius(context.level().getRandom(), 5, 42);
			grid[coords.getX()][coords.getZ()] = 1;
		}
	...
	@Override
	public Optional<GeneratorResult<ChestGeneratorData>> generate(IFeatureGenContext featureGenContext, ICoords spawnCoords, IRarity rarity, ChestRarity chestRarity, int[][] grid, List<ICoords> queuedChunks) {

		// TODO for all the queued grids

		// TODO check that the chunk is loaded

		// TODO every chunk needs to check if valid surface pos

		// TODO if generating the main chunk, then do all the checks - check the underground, etc.

		return Optional.empty();
	}

	// TODO doesn't implement anything
	// TODO needs to take in the 2D grid(s)
	// TODO is spawnCoords the center of the grid x,z ? - it should be the center ie the main tree spawn
	@Override
